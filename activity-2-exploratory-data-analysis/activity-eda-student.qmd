...
```
After reading in the data, it is a good habit to always run the function `names()` on our dataset. This will print a list of the variable/column names. *Remember: R is case-sensitive, so we should note any capitalization!* Insert a code chunk and run the code `names(penguins)` to print out the variable names in the `penguins` dataset.
